...
// 获得需要删除的文件名列表
func GetRemoveFiles(dirpath string,s S_Files,daynum int)(fpath []string){
    if len(s) > 0 {
        f1 := s[0].ModTime()
        ...
        for _,f := range s {
            ...
            fname := f.Name()
            if strings.Contains(fname,date) { //仅清理包含日志日期标志的文件
...
func removeLogFiles(dir string)( lasterr error){
	...
	rfiles := GetRemoveFiles(dir,files,removeLogFilePrevDay)
